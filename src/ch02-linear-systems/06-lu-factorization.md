...
The factorization stores about half as much: \( \L \) and the diagonal \( \D \) determine everything. Over \( \nR \), when every entry of \( \D \) is positive, one can split \( \D = \D^{1/2}\D^{1/2} \) and write \( \A = (\L\D^{1/2})(\L\D^{1/2})\tp \); this is the **Cholesky factorization**, which Chapter 12 connects with positive definite matrices.
