...
::: {.remark}
Many books draw the arrow for \( a_{ij} \ne 0 \) from \( i \) to \( j \). The two graphs differ by reversing every edge, and nothing below depends on the choice except the wording "from \( j \) to \( i \)". We take the direction that matches \( \x_{k+1} = \A\x_k \) and the column-stochastic convention of Chapter 8 §11.
:::
