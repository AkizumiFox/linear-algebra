...
::: {.remark}
This is not a proof from nothing. The root bound of Chapter 6 (@cor-root-bound-general) did the real work inside @thm-vandermonde-determinant, exactly as the root bound did in Chapter 3 (@lem-root-bound). What the determinant adds is an explicit number: it says how close to singular the interpolation problem is, since nodes that nearly coincide make the product small.
:::
...
Circulant matrices of every size are treated in Chapter 12.
::::
