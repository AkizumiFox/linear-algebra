...
**What you need.** From Chapter 0, permutations and transpositions (@thm-transpositions-generate) and the characteristic of a field (@def-characteristic). From Chapter 1, linear independence and the Linear Dependence Lemma (@def-linear-independence, @thm-linear-dependence-lemma). From Chapter 3, elementary matrices and the invertible matrix theorem (@def-elementary-matrix, @thm-invertible-tfae). From Chapter 2, the matrix of a linear map and similarity (@def-matrix-of-linear-map, @thm-similar-iff-same-operator). From Chapter 5, only the word **linear functional** (@def-linear-functional), which §02 borrows to say what a multilinear form is in each argument separately; no result of that chapter is used in a proof here. From Chapter 6, the root bound for polynomials (@cor-root-bound-general).
