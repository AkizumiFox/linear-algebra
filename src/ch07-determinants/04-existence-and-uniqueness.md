...
2. Gaussian elimination (Chapter 3) brings \( \A \) to upper triangular form. Suppose no row swaps are needed. At step \( k \) (for \( k = 1, \dots, n - 1 \)), each of the \( n - k \) rows below the pivot requires one division to find its multiplier and \( n - k \) multiplications to update its entries in columns \( k + 1, \dots, n \). Show that the total number of multiplications and divisions is \( \frac{(n-1)n(n+1)}{3} \), and evaluate it for \( n = 20 \).
:::
