...
To compare two bases of an arbitrary real space, we compare them with each other rather than with a fixed standard basis, using the change-of-coordinates matrix of Chapter 4.
...
For \( \u, \v \in \nR^3 \), let \( \A = \begin{pmatrix} \u & \v \end{pmatrix} \in M_{3 \times 2}(\nR) \). We **define** the area of the parallelogram \( P(\u, \v) \subseteq \nR^3 \) to be \( \sqrt{\det(\A\tp \A)} \); Chapter 13 justifies this with inner products, where \( \det(\A\tp\A) \) is recognized as a Gram determinant.
