...
Now the connection this chapter was built for. Chapter 9 §10 defined convergence of matrices **entrywise** (@def-entrywise-convergence) and said, in its opening paragraph, that no notion of the size of a matrix was used and that Chapter 15 would supply one. Here is the reconciliation.
...
::: {.check}
On \( C[0,1] \), compare the sup norm \( \norm{f}_\infty \) of @exm-induced-and-weighted-norms (c) with \( \norm{f}_1 = \int_0^1\lvert f\rvert \), which is a norm because a continuous \( f \ge 0 \) with \( \int_0^1 f = 0 \) is the zero function — the one fact from analysis that Chapter 10 §01 recorded when it put an inner product on \( C[0,1] \), quoted again here. Which of the two inequalities of @def-equivalent-norms holds, and which fails?
:::
