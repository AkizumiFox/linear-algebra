...
::: {.proof}
The supremum exists for each \( \x \in C \) by completeness, fact (A1) of Chapter 15's introduction. Let \( \x, \y \in C \) and \( 0 \le t \le 1 \). For each \( i \in I \), by @exm-first-convex-functions (a) and then \( a_i \le g \) at \( \x \) and at \( \y \),
\[
...
This is the fact Chapter 16 §06 anticipated, and its promise is now paid. Chapter 16 reached convexity by a different path: @cor-ky-fan-subadditive gives \( s_k(\A + \B) \le s_k(\A) + s_k(\B) \), and positive homogeneity \( s_k(t\A) = ts_k(\A) \), \( t \ge 0 \), turns subadditivity into convexity. Exercise C1 shows that the two paths are the same, and the warning at the end of Chapter 16 §06 still stands: a single \( \lambda_k \) with \( 1 < k < n \), the difference \( s_k - s_{k-1} \) of two convex functions, is neither convex nor concave.

**What is not proved here.** For a convex \( f \) on an interval, \( \A \mapsto \tr f(\A) = \sum_i f(\lambda_i(\A)) \) is convex on the Hermitian matrices with spectrum in that interval, a statement about sums over the spectrum and far weaker than the operator convexity of Chapter 16 §11. It is proved in Chapter 20, and nothing in this chapter uses it.
...
\]
By the mean value theorem, fact (A6) of Chapter 15's introduction, applied on \( [x, z] \) and on \( [z, y] \), there are \( c_1 \in (x, z) \) and \( c_2 \in (z, y) \) with
\[
...
**Step 1: the second difference.** We claim \( \Delta(\z, \v, s)/s^2 \to \v\tp\H(\z)\v \) as \( s \to 0 \). If \( \v = \0 \) both sides are \( 0 \). Otherwise Chapter 13 §06's (A2) at \( \z \), with \( \h = s\v \) and with \( \h = -s\v \), gives
\[
...
Suppose \( h(t_1) \ge 0 \) for some \( t_1 \in (0, 1) \). The interval \( [0, 1] \) is closed and bounded, hence compact by fact (A3) of Chapter 15's introduction, so by the extreme value theorem, fact (A4), \( h \) attains a maximum value \( M \) at some point. Then \( M \ge h(t_1) \ge 0 \). If the maximum is attained only at \( 0 \) or \( 1 \), then \( M = 0 \le h(t_1) \), so \( t_1 \) attains it too. Either way, \( M = h(t^{*}) \) for some \( t^{*} \in (0, 1) \). For \( 0 < s < \min(t^{*}, 1 - t^{*}) \), both \( h(t^{*} \pm s) \le h(t^{*}) \), so the second difference of \( h \) at \( t^{*} \) is \( \le 0 \), and so is its limit \( L(\x + t^{*}\v) \). This contradicts \( L > 0 \). Hence \( h < 0 \) on \( (0, 1) \), which is the claim.
...
**Step 3: continuity.** If \( \x_k \to \a \), then \( \norm{\x_k - \a} \le \delta \) for all large \( k \), and for those \( k \) Step 2 gives \( \lvert f(\x_k) - f(\a)\rvert \le L\norm{\x_k - \a} \to 0 \). So \( f \) is continuous at \( \a \), in the sequential sense of Chapter 15's introduction. As \( \a \) was arbitrary, this proves the theorem.
:::
...
(f) Chapter 13 §06's assumptions (A1), the symmetry of the Hessian, and (A2), Taylor's theorem with a second-order remainder; facts (A3) and (A4) of Chapter 15's introduction, compactness of \( [0, 1] \) and the extreme value theorem, in the maximum principle; and the algebra of limits, in the form "a non-strict inequality survives a limit".
:::
