...
\norm{\x - \y} \le \norm{\x - \y_k} + \norm{\y_k - \y} \le R + \norm{\y_k - \y}
\]
for every \( k \); the right-hand side tends to \( R \), and a non-strict inequality survives a limit, so \( \norm{\x - \y} \le R \). By @cor-closed-bounded-compact, the form in which Chapter 15 carried **the compactness of closed bounded sets, fact (A3) of Chapter 15's introduction**, over to every finite-dimensional space, \( K \) is compact. The function \( f(\y) = \norm{\x - \y} \) is continuous, since \( \lvert f(\y) - f(\y')\rvert \le \norm{\y - \y'} \) by @lem-reverse-triangle-norm. By **the extreme value theorem, fact (A4) of Chapter 15's introduction**, \( f \) attains a minimum on \( K \) at some \( \p \in K \). Every \( \y \in C \setminus K \) has \( f(\y) > R \ge f(\p) \). So \( \norm{\x - \p} \le \norm{\x - \y} \) for every \( \y \in C \).
...
The inequality \( (\ast) \) is Pythagoras with the equality relaxed to an inequality. That is exactly the relation with Chapter 10, and it deserves to be said precisely.
...
\]
So \( P_C(\x) = \x - \lambda\a \): drop the perpendicular to the boundary hyperplane, as Chapter 10 would have done. The distance is \( \lambda\norm{\a} = (\inner{\a}{\x} - c)/\norm{\a} \).
:::
...
A hyperplane in the sense of Chapter 4 passes through \( \0 \): it is a subspace of codimension \( 1 \) (@def-hyperplane). To separate sets we need translates of hyperplanes, and Chapter 4 §06 called a coset \( \p + H \) of a hyperplane \( H \) (@def-coset) an **affine hyperplane**. In an inner product space these have a concrete form. By @thm-hyperplane-kernel-functional, \( H = \ker\varphi \) for a non-zero \( \varphi \in V^{*} \); by @thm-riesz-representation, \( \varphi = \inner{\cdot}{\a} \) for a non-zero \( \a \in V \). Hence
\[
...
The unit sphere of \( V \) is compact by @cor-closed-bounded-compact, which rests on **fact (A3) of Chapter 15's introduction**. So a subsequence \( (\a_{k_j}) \) converges to some \( \a \) with \( \norm{\a} = 1 \); in particular \( \a \ne \0 \). Fix \( \y \in C \). Then \( \inner{\a_{k_j}}{\y - \x_{k_j}} \le 0 \) for every \( j \), and \( \x_k \to \x_0 \) because \( \norm{\x_k - \x_0} = \norm{\x_0 - \z}/k \). By @thm-cauchy-schwarz,
\[
\begin{aligned}
...
\]
that is, \( f(\u) - p(\u - \v) \le p(\u' + \v) - f(\u') \). So every number in \( L = \{f(\u) - p(\u - \v) : \u \in U\} \) is at most every number in \( R = \{p(\u' + \v) - f(\u') : \u' \in U\} \). By **fact (A1) of Chapter 15's introduction**, the non-empty set \( L \), bounded above by any element of \( R \), has a least upper bound \( c \), and \( c \le r \) for every \( r \in R \). With this \( c \), we check \( g(\u + t\v) \le p(\u + t\v) \) in three cases.
...
2. Which two facts of Chapter 15's introduction does the existence of \( P_C(\x) \) use, and to which set are they applied?
