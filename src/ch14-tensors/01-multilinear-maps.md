...
(g) The cross product of Chapter 6 has components \( a_2b_3 - a_3b_2 \) and its two cousins, each bilinear as in @exm-alternating-forms (b). A vector of bilinear functions is bilinear, since linearity in a slot may be checked coordinate by coordinate. So it is bilinear, with values in \( \nR^3 \) — an example that genuinely needs @def-multilinear-map rather than @def-multilinear-form.
:::
...
(d) Both are \( mn \). With \( k = 1 \) the two spaces are equal, by @exm-multilinear-maps-first (a), and @eq-multilinear-dimension reduces to @thm-linear-maps-isomorphic-to-matrices. The formula is consistent with what Chapter 3 already proved.
:::
