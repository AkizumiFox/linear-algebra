...
Chapter 13 §12 defined a Clifford algebra of \( (V, q) \) as a pair \( (C, \iota) \) with \( \iota \colon V \to C \) linear satisfying
...
Chapter 13 proved half of the count. Its @prp-clifford-spanning showed that for an orthogonal basis \( (\e_1, \dots, \e_n) \) of \( \beta \) the \( 2^n \) **sorted products**
\[
...
**The debt, and how it was paid.** Chapter 13 §12 stated @thm-clifford-dimension and then wrote, under the heading "What is proved here, and what is not", that the inequality \( \dim_F C \le 2^n \) was proved in full, while "the reverse inequality — that the \( 2^n \) sorted products are linearly independent — is **not proved in this section**", and that the two halves would have to be established together by exhibiting one algebra of dimension exactly \( 2^n \) satisfying (C1) and (C2). Both halves are now supplied. The missing one was independence, and @thm-clifford-dimension-proved supplies it; the prior question, whether any Clifford algebra exists at all, is @thm-clifford-exists. Chapter 13 read the statement for \( n \ge 3 \) "as quoted from Chapter 14 and not as proved here"; it may now be read as proved.

The prediction was right in substance and slightly off in shape. The algebra of dimension \( 2^n \) that settled the matter is the exterior algebra, but it is not the Clifford algebra: it is only a space that \( \operatorname{Cl}(q) \) acts on, and independence came from the action rather than from an identification. Chapter 13 also guessed that "\( \operatorname{Cl}(q) \) is assembled from the exterior powers \( \Lambda^kV \), one for each \( k \)", and \( \Phi \) is that assembly, matching the sorted product on \( S \) with the wedge \( \e_S \).
...
::: {.solution}
Twice, both times through the orthogonal basis. It is needed to produce one at all: @thm-symmetric-form-diagonalizable assumes \( \operatorname{char} F \ne 2 \). And it is needed for Chapter 13's @prp-clifford-spanning, which supplies the spanning half, since that proof uses @lem-clifford-anticommutation, which comes from polarization. The operator identities of @lem-clifford-operators and the induction of Step 3 use no such hypothesis.
:::

With that, Chapter 13's last open statement is closed and the construction side of this chapter is complete. The final section spends the machinery on one application, where the failure of decomposability proved above reappears under another name.
...
4. Which half of @thm-clifford-dimension was left unproved in Chapter 13, and which result of this section supplies it?
