...
Two backward maps are now attached to \( T \): the dual map \( T' \colon W^{*} \to V^{*} \) of Chapter 4, which needs no inner product, and the adjoint \( T^{*} \colon W \to V \), which does. The Riesz map converts one into the other.
