...
The two answers of this section — the least-squares solution and the minimum-norm solution — are usually wanted together: given any \( \A \) and \( \b \), first minimize \( \norm{\A\x - \b} \), then among the minimizers take the shortest. Chapter 13 packages the combined answer as a single matrix, the **pseudoinverse** \( \A^{+} \), with \( \x = \A^{+}\b \) in all cases at once.
