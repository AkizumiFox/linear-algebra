...
Over \( \nR \), with \( \cA = V \), a flat is exactly Chapter 18's affine subspace of \( V \) (@def-affine-hull (b)), and the two dimensions agree: Chapter 18 also reads the subspace off the coset as the set of differences. Chapter 5's **affine hyperplane** \( \p + H \) (the paragraph before @prp-affine-subspace-intersection) is the case \( \dim A = n - 1 \) in \( \nA^n(F) \). A one-point set \( \{P\} = P + \{\0\} \) is a flat, and \( \cA = P + V \) is a flat: the two degenerate cases.
...
The intersection contains \( A \), so it is non-empty, and it is a flat by @prp-flat-intersection: \( A \vee B \) is the **smallest** flat containing both. A single point \( P \) is a flat, and we write \( P \vee Q \), \( A \vee P \) for joins in which a one-point flat \( \{P\} \) appears. This is the affine analogue of \( U + W \), and Chapter 18's affine hull is the same construction over \( \nR \) (@prp-affine-hull-coset).
...
Over \( \nR \) with \( \cA = V \) this is @lem-affine-dependence (a), and Chapter 18's warning applies unchanged: affine independence is **not** linear independence. The list \( (\0, \e_1, \e_2) \) in \( \nR^2 \) is linearly dependent and affinely independent, which is why the count below is \( n + 1 \) and not \( n \).
...
Chapter 3 produced flats as solution sets; Chapter 5 proved that every subspace of \( F^n \) is a null space. Putting the two together identifies the flats of \( \nA^n(F) \) completely.
...
::: {.idea}
A flat is a coset \( \p + U \) by @prp-flat-is-coset, Chapter 5 turns \( U \) into a null space, and Chapter 3's "particular plus homogeneous" turns the coset into the solution set of the inhomogeneous system with the same matrix and right-hand side \( \M\p \).
:::
