...
Part (c) is the useful half. Translating the origin to a center — that is, applying @prp-quadric-congruence with \( \M = \I \) and \( \t = \p \) — gives \( \b' = \A\p + \b = \0 \), so the equation becomes \( \v\tp\A\v + Q(\p) = 0 \) with no linear term at all, and Chapter 14 applies verbatim. That single move is the engine of the classification.
...
The gain is immediate. Under \( \x = \U\y + \t \) the small matrix becomes \( \U\tp\A\U \), which is both a congruence and a **similarity**, since \( \U\tp = \U^{-1} \). So the eigenvalues of \( \A \), not merely their signs, are Euclidean invariants. This is the observation Chapter 12 made in @thm-principal-axes, and the classification below is that theorem with the origin allowed to move.
