...
\]
and the lower end is \( \theta_1 \) itself by interlacing. The bracket \( [3.9318,\ 4.0260] \) has width \( 0.094 \) where the residual bound gave \( 0.593 \). That is the quadratic gain of Chapter 19 §06, obtained here from one number the process had already produced.
:::
...
3. Hence give a second proof of @thm-hessenberg-form over \( \nC \) for such \( \A \), and explain why it does not replace Chapter 10's.
:::

*Hint for (a): Chapter 9 §05 calls such a \( \b \) a cyclic vector.*
:::
