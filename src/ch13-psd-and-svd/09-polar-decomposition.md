...
The polar factor is not merely *a* unitary matrix attached to \( \A \). It is the one closest to \( \A \), measured in the Frobenius norm of Chapter 12 §11. This is the reason the polar decomposition is used in practice: given a matrix that ought to be unitary but has drifted, \( \W \) is how to put it back.
...
\norm{\A - \Q}_F = \norm{\U^{*}(\A - \Q)\V}_F = \norm{\vSigma - \Z}_F .
\]
Expanding the square in the Frobenius inner product \( \inner{\X}{\Y} = \tr(\Y^{*}\X) \) of Chapter 11 §01,
...
\]
so @eq-nearest-unitary-expand gives \( \norm{\A - \Q}_F^2 \ge \sum_i\sigma_i^2 + n - 2\sum_i\sigma_i \) for every unitary \( \Q \). Taking \( \Q = \W = \U\V^{*} \) makes \( \Z = \U^{*}\U\V^{*}\V = \I_n \), for which \( \operatorname{Re}z_{ii} = 1 \) for every \( i \) and the bound is attained. Finally \( \norm{\A}_F = \norm{\U\vSigma\V^{*}}_F = \norm{\vSigma}_F \) by @lem-frobenius-unitarily-invariant, applied once on each side, and the only non-zero entries of \( \vSigma \) are \( \sigma_1, \dots, \sigma_n \) on its diagonal, so \( \norm{\A}_F^2 = \sum_i\sigma_i^2 \) (@exr-singular-value-decomposition-c1). The minimum value is therefore as stated.
...
::: {.idea}
Four steps, and the first two are the polar decomposition doing the work. ① Symmetry makes \( \A \) intertwine \( \A^{*}\A \) with its conjugate: \( \A(\A^{*}\A) = \overline{(\A^{*}\A)}\A \). Because the positive square root is a polynomial in the matrix, the same relation passes to \( \lvert\A\rvert \). ② That relation, combined with \( \A\tp = \A \), forces the polar factor \( \W \) to be a **symmetric** unitary matrix. ③ A symmetric unitary matrix factors as \( \V\V\tp \): split it into real and imaginary parts, which turn out to be commuting real symmetric matrices, and use Chapter 12's simultaneous diagonalization (@cor-simultaneous-orthogonal-real). ④ Conjugating \( \A \) by that \( \V \) in the right way produces a matrix that is both symmetric and Hermitian, hence **real** symmetric, and the real spectral theorem in matrix form (@cor-spectral-real-matrix) finishes the job. We prove all of this for invertible \( \A \), where the polar factor is unique and \( \lvert\A\rvert^{-1} \) exists.
:::
