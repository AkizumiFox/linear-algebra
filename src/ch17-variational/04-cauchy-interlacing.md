...
The extreme case \( m = n-1 \) is worth writing down, because it recovers a fact about diagonal entries that also follows from Chapter 13's description of the extreme eigenvalues.
...
::: {.proof}
(a) Put \( D_k(x) = \det(x\I_k - \T_k) \) and \( D_0 = 1 \). For each real \( x_0 \), the matrix \( x_0\I_k - \T_k \) is tridiagonal in the sense of Chapter 7, with diagonal entries \( x_0 - a_{i-1} \) and off-diagonal entries \( -\sqrt{b_i} \) in both positions \( (i, i+1) \) and \( (i+1, i) \), so @thm-tridiagonal-recurrence gives \( D_1(x_0) = x_0 - a_0 \) and
\[
\begin{aligned}
