...
The derivation above suggests the proof: manufacture \( \S(t) \) and conclude by similarity. That route runs into an obstacle the book cannot clear. The natural \( \S \) is the solution of \( \S' = \B\S \), \( \S(t_0) = \I \), and for a **constant** \( \B \) that is \( e^{(t-t_0)\B} \) by Chapter 10 §09. For a \( \B \) that depends on \( t \) — the only case of interest here — the existence of such an \( \S \) is a theorem about linear differential equations with variable coefficients, and this book does not prove it. So we take a different route, which needs no flow at all.
...
A differentiable function on an interval with vanishing derivative is constant — the mean value theorem, fact (A6) of Chapter 16's introduction, applied to the real and imaginary parts of \( f \). (Chapter 10 §09's fact (A4) says this for functions defined on all of \( \nR \), which is not the hypothesis here.) So \( p_{\L(t)}(x) = p_{\L(s)}(x) \) for all \( s, t \in I \).
...
\end{aligned}
\]
By fact (A4) of Chapter 10 §09, every entry of \( \M \) is constant, so \( \M(t) = \M(0) = \L(0) \). Multiplying on the left by \( e^{t\B} \) and on the right by \( e^{-t\B} \), and using \( (e^{t\B})^{-1} = e^{-t\B} \) from @thm-exponential-properties (c), gives the formula. This proves the corollary.
:::
...
\tr\L'(t) = \tr(\B\L) - \tr(\L\B) = 0
\]
by @thm-trace-properties (3). So \( \tr\L \) has zero derivative on \( I \) and is constant by fact (A6) of Chapter 16's introduction. The same argument applied to \( \L^m \) gives \( \tr(\L^m)' = 0 \) for every \( m \ge 1 \) (@exr-lax-pairs-and-the-toda-flow-c1).
:::
...
\]
by (D1) of Chapter 16 §06. Substituting,
\[
...
\]
and the two scalar series are the even and odd halves of \( e^{t} = \sum_j t^j/j! \), namely \( c(t) \) and \( s(t) \) by fact (A1) of Chapter 10 §09.
...
\]
Now \( \tr(\L^{m-1}\B\L) = \tr(\L\L^{m-1}\B) = \tr(\L^{m}\B) \) by a cyclic shift, and \( \tr(\L^{m-1}\L\B) = \tr(\L^m\B) \) as well, so the bracket contributes \( 0 \). By fact (A6) of Chapter 16's introduction the function is constant on \( I \).
