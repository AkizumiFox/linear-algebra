...
(b) Two-sided, because \( F[x] \) is commutative. Chapter 6's (I1)–(I3) are exactly @def-left-ideal read in a commutative algebra, with (I1) and (I2) replaced by the stronger requirement that \( I \) be a subspace — which is no change, since \( cf = (c1)f \in I \) for a scalar \( c \) by (I3).
...
The non-example is the one Chapter 15 already gave, and it is worth repeating because it is the mistake to avoid. The diagonal matrices in \( M_2(F) \) form a subspace closed under products — a subalgebra, in fact — and are **not** an ideal of any kind: \( \E_{11} \) is diagonal, while \( \E_{21}\E_{11} = \E_{21} \) and \( \E_{11}\E_{12} = \E_{12} \) are not. Closure under one's own products is not absorption.
...
Chapter 15 §10 did the work. @lem-quotient-algebra says that the quotient **space** \( A/\cI \) of Chapter 2 (@def-quotient-space) carries exactly one multiplication making \( \pi \colon A \to A/\cI \) multiplicative, namely \( (x + \cI)(y + \cI) = xy + \cI \), and that with it \( A/\cI \) is an algebra with identity \( 1_A + \cI \). We give the object its name.
...
::: {.idea}
Chapter 4 already produced the linear isomorphism; all that is left is to see that it respects products, and that is one line, because the product on the quotient was **defined** so that \( \pi \) is multiplicative.
:::
