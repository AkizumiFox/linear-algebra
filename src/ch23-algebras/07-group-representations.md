...
The book's groups so far have been groups *of* matrices or *of* permutations: \( \GL_n(F) \) (@exm-groups), the symmetric group \( S_n \), the classical groups of Chapter 14. Such a group arrives with its linear algebra attached. An abstract group does not. If all we know about \( G \) is its multiplication table, the only way to bring linear algebra to bear is to manufacture operators out of it.
...
Having made operators, we ask the question this book always asks of a family of operators: can the space be broken into smaller pieces that the family preserves? Chapter 9 called such a piece an invariant subspace (@def-invariant-subspace). Here the family is \( \{\rho(g) : g \in G\} \).
...
::: {.remark}
Working with \( F[G] \) rather than with \( G \) buys one thing that matters: the ability to **add** group elements. Every averaging argument below lives in \( F[G] \), not in \( G \). Chapter 10 §07 made the same move for a single operator, replacing \( T \) by the algebra \( F[T] \) and the space by an \( F[x] \)-module.
:::
