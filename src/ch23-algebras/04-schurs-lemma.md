...
We have objects: \( A \)-spaces. We have subobjects: invariant subspaces. What is missing is the maps, and the requirement almost writes itself. Chapter 9 §09 got everything it had out of a single hypothesis, \( ST = TS \); read \( S \) as a map and \( T \) as the action, and that hypothesis is the one below.
