...
The Factor Theorem peels off one root at a time. Doing it for several distinct roots at once strengthens the Root bound of Chapter 2 (@lem-root-bound), which said that a non-zero polynomial of degree at most \( n \) has at most \( n \) distinct roots. The stronger version says **why** there are so few: every root contributes its own linear factor.
...
Chapter 0 warned that a polynomial is not the function it defines: over \( \nF_2 \), the non-zero polynomial \( x^2 + x \) has the zero function (@exm-polynomial-vs-function-f2). It also promised that over an infinite field the distinction disappears. Chapter 2 kept that promise in a paragraph right after @lem-root-bound. We record the fact here as a theorem, in a slightly sharper form that counts how many values are needed.
