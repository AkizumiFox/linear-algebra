...
Chapter 3 built projections out of nothing but a direct sum: given \( V = U \oplus W \), the operator that keeps the \( U \)-piece and discards the \( W \)-piece satisfies \( P^2 = P \), and every operator satisfying \( P^2 = P \) arises this way. Chapter 10 built a second kind by taking \( W = U^{\perp} \), the orthogonal projection \( P_U \). The second is a special case of the first, and Chapter 3 left the comparison open, saying only that its projections "are often called **oblique** projections, to distinguish them from the orthogonal projections met later, in the chapters on inner product spaces". This section makes the distinction exact, measures how far an oblique projection is from being orthogonal, and closes Part III.

Throughout, \( V \) is a finite-dimensional inner product space over \( F = \nR \) or \( \nC \), and \( n = \dim V \).

## Which projections are orthogonal?

Two recollections, because the section is about the gap between them. From Chapter 3: an operator \( P \in \cL(V) \) with \( P^2 = P \) is a **projection**, also called an **idempotent** (@def-projection-operator); it satisfies \( V = \im P \oplus \ker P \), it is the identity on \( \im P \) and zero on \( \ker P \), and each splitting \( V = U \oplus W \) has exactly one projection with image \( U \) and kernel \( W \) (@thm-projection-direct-sum). No inner product appears there, and the field was arbitrary.

From Chapter 10: the splitting \( V = U \oplus U^{\perp} \) of @thm-orthogonal-decomposition is one of the many available, and \( P_U \) is the projection belonging to it (@def-orthogonal-projection). Chapter 3 warned that a subspace is the image of infinitely many projections, one per complement. Which one does the inner product pick out, and how is it recognized without being told the kernel?
...
The theorem is the promised comparison. Every projection of Chapter 3 that is **not** self-adjoint is oblique, and now "oblique" has a precise meaning: its kernel is a complement of its image that is not the perpendicular one. Chapter 11 has already met a pair of such projections, in @exm-oblique-projections, and the Quick check after @thm-spectral-resolution-unique traced exactly what their obliqueness cost.

Clause (c) also settles Chapter 3's warning that "projection onto \( U \)" is incomplete information.
...
What comes next takes the quadratic form \( \x \mapsto \inner{\A\x}{\x} \), which was only a test in this chapter, and makes it the object of study: Chapter 13 classifies forms up to congruence, by inertia, over \( \nR \) and over \( \nC \). Chapter 15 gives the operator norm promised twice above, and Chapter 16 characterizes the eigenvalues of a Hermitian matrix as maxima and minima of \( \inner{\A\x}{\x} \) — at which point the test becomes the theory.
