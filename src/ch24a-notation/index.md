...
**Indices start at 1**, with three announced exceptions: Chapter 12 §09, where circulant and Fourier subscripts run from \( 0 \) to \( n-1 \) and are read modulo \( n \); Chapter 22 §§06 to 10, where the coordinates of \( F^{n+1} \) run from \( 0 \) so that \( \nP^n(F) \) has dimension \( n \) (@def-homogeneous-coordinates); and Chapter 23 §09, which follows Chapter 12 §09 for group elements and characters.
...
| \( e_i \) versus \( \e_i \) | italic \( e_i \) is an idempotent of a product of algebras (@def-product-of-algebras); bold \( \e_i \) is the \( i \)-th standard basis vector (@exm-standard-bases) | the typeface, and that is the whole reason Chapter 23 chose italic |
...
| \( \L \) | the lower triangular factor of an elimination (@def-lu-factorization); the graph Laplacian (@def-graph-laplacian); the Lax matrix (@def-lax-pair). Plain \( L_a \) is left multiplication in an algebra (@thm-cayley-for-algebras) and \( L_f \) a Loewner matrix (@def-loewner-matrix); calligraphic \( \cL(V,W) \) is a space of maps (@def-space-of-linear-maps) | Chapter 24 §§08 and 11 each declare their use at the head of the section; the plain letters carry a subscript |
| \( \R \), \( r \) | the \( \R \) of a \( QR \) factorization; the plane rotation \( \R_\theta \) (@thm-orthogonal-2x2); the rotation through \( 2\pi/n \) in Chapter 23; the absorbing block of a Markov chain (@def-absorbing-chain). Plain \( R_{\A}(\x) \) is the Rayleigh quotient (@def-rayleigh-quotient) and \( r_i(\A) \) a Gershgorin radius (@def-gershgorin-discs) | the Rayleigh quotient is plain italic and takes a vector; the Gershgorin radius is plain and takes an index |
| \( \Q \) | the orthogonal factor of a \( QR \) factorization; the orthogonal linear part of a Euclidean motion (@def-euclidean-motion); the quarter turn in Chapter 23; the transient block of an absorbing chain (@def-absorbing-chain). Blackboard \( \nQ \) is the rationals (@def-common-number-sets) | the chapter, and the blackboard face for the field |
| \( \S \), \( \cS \) | a congruence or a similarity matrix; a Schur complement in Chapter 24 §02; the sample covariance (@def-sample-covariance); the cyclic shift in Chapter 12 §09. Calligraphic \( \cS_{\A,\B} \) is the Sylvester operator (@def-sylvester-operator) | each section of Chapter 24 says which it means; the Sylvester operator carries both its matrices |
...
| \( \C \) | the companion matrix \( \C(p) \) (@def-companion-matrix); the real Jordan block \( \C_k(\lambda) \) (@def-real-jordan-block); a circulant (@def-circulant). Plain \( C \) and \( C^a_b \) are contractions (@def-contraction) and \( C_{ij} \) a cofactor (@def-minor-cofactor); blackboard \( \nC \) is the complex numbers | the companion matrix always carries its polynomial, the Jordan blocks their size and eigenvalue; a bare \( \C \) in Chapter 23 is a circulant |
| \( \W \), \( W \) | the Wielandt matrix \( \W_n \) (@thm-wielandt-bound); the unnormalized transform matrix \( \W_n \) of Chapter 24 §12; the accumulated orthogonal factor \( \W_k \) of Chapter 24 §08. Plain \( W \) is a subspace, \( W(\A) \) the numerical range (@def-numerical-range), and \( W_n \) the zero-sum matrices of Chapter 19 §07 (@lem-doubly-stochastic-sum-conditions) | the numerical range always carries its matrix as an argument; the subspace never carries one |
...
| \( \Pi \), \( \pi \) | bold \( \vPi \) is the limit of a convergent stochastic matrix (@thm-matrix-powers-converge); plain \( \Pi \) is a plane in Chapter 22 §§04, 08, 09 and the quotient map \( \GL(V) \to \operatorname{PGL}(V) \) in §07; \( \pi \) is the quotient map \( X \to X/{\sim} \) of a set (@def-quotient-set), a quotient homomorphism of algebras (@def-quotient-algebra) and, locally, a slot projection | the typeface, and that no section of Chapter 22 uses both meanings |
| \( \Phi \), \( \Psi \) | a symmetric gauge function and its dual (@def-symmetric-gauge, @def-dual-gauge); the two isomorphisms of Chapter 23 §09 (@def-fourier-transform-on-a-group) | the argument: a gauge eats a real vector, Chapter 23's maps eat an element of a group algebra |
...
| \( \lambda \), \( \mu \) | a scalar or an eigenvalue throughout; also the linear-part homomorphism \( \operatorname{E}(n) \to \Orth(n) \) (@def-euclidean-motion) and the Fiedler value \( \mu_2 \) (@def-fiedler-value) | Chapter 22 §04 is the one place \( \lambda \) is not a scalar, and it says so |
| \( \lvert \cdot \rvert \) | modulus of a complex number (@def-conjugate-modulus); cardinality of a set; the matrix absolute value \( (\A^{*}\A)^{1/2} \) (@def-matrix-absolute-value); the **entrywise** absolute value in Chapter 19 (@def-entrywise-order) | what sits inside. Chapter 19 states its override, and the Chapter 13 meaning does not appear there |
...
| \( u \) | the unit roundoff (@def-floating-point-model); bold \( \u \) is a vector | the typeface; Chapter 24 §01 warns about the pair |
| \( h \) | the period of an irreducible non-negative matrix (@def-period); a support function \( h_K \) (@def-support-function); a mesh width in Chapter 19 §09 | the support function always carries its convex set |
