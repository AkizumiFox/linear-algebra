...
::: {.proof}
Suppose not. Then for some \( \varepsilon > 0 \) and every \( k \ge 1 \) the value \( \delta = 1/k \) fails, so there are \( s_k, s'_k \in [a, b] \) with \( \lvert s_k - s'_k \rvert < 1/k \) and \( \norm{\M(s_k) - \M(s'_k)}_F > \varepsilon \). The interval \( [a, b] \) is a closed bounded subset of \( \nR \), so by the compactness of closed bounded sets, **fact (A3) of Chapter 15's introduction**, a subsequence \( s_{k_j} \) converges to some \( s \in [a, b] \). Then \( s'_{k_j} \to s \) as well, since \( \lvert s'_{k_j} - s \rvert \le \lvert s'_{k_j} - s_{k_j}\rvert + \lvert s_{k_j} - s \rvert \to 0 \). Continuity of \( \M \) gives \( \M(s_{k_j}) \to \M(s) \) and \( \M(s'_{k_j}) \to \M(s) \), so \( \norm{\M(s_{k_j}) - \M(s'_{k_j})}_F \to 0 \), contradicting the lower bound \( \varepsilon \).
:::
...
Two statements in this chapter are not proved from Chapters 1–19: the one below, and Lieb's concavity theorem, which §12 states and nothing in the book uses. Beside them this section quotes two standard facts of one-variable calculus, the fundamental theorem of calculus and the substitution rule, each named at the point where it is used. The book's practice for such a fact is fixed — Chapter 10 with the theorem of Abel and Ruffini, Chapter 13 §06 with Taylor's theorem — and it is followed here: the statement is given in full, in a block of its own, named at every use, and never buried inside a proof that presents itself as self-contained.
...
Chapter 12 §05 announced the answer and deferred the proof:

> The dividing line is at \( s = 1 \): the functions preserving the order are called **operator monotone**, and \( t \mapsto t^{s} \) is operator monotone on \( [0, \infty) \) exactly for \( 0 \le s \le 1 \). Chapter 20 proves this, along with the companion fact that \( t \mapsto \log t \) is operator monotone while \( t \mapsto e^{t} \) is not.

Chapter 16 §11 repeated it, adding that "the operator monotonicity of \( t^p \) for every \( p \in (0, 1) \) needs a new idea, an integral representation of the kind in @thm-loewner-statement". Here is the representation, and it is elementary. Real powers \( t^{r} = e^{r\log t} \) are Chapter 17 §11's (@lem-exp-log).
...
::: {.remark}
The number \( K_p \) equals \( \pi/\sin(p\pi) \). That evaluation is a standard exercise in contour integration, and the constant \( \sin(p\pi)/\pi \) is the one Chapter 16 §11 displayed. Nothing below needs its value: only \( 0 < K_p < \infty \) is used, and that was proved. Exercise B2 computes \( K_{1/2} = \pi \) by hand.
:::
...
::: {.idea}
For \( 0 < p < 1 \), @lem-power-integral rewrites \( t^{p} \) as a positive superposition of the functions \( t/(t+s) \) that Chapter 16 §11 already proved operator monotone, and the superposition is an honest Riemann integral, so @lem-integral-preserves-loewner finishes it. For \( p > 1 \) the trick is that operator monotone functions compose: if \( t^{p} \) were operator monotone for one \( p > 1 \), enough compositions with each other and with a small power would manufacture \( t^{2} \), which is not.
:::

::: {.proof}
**The cases \( p = 0 \) and \( p = 1 \).** These are the constant \( 1 \) and the identity, operator monotone by @prp-affine-operator-monotone.

**The case \( p < 0 \).** Then \( t \mapsto t^{p} \) is strictly decreasing on \( (0, \infty) \), since \( t \mapsto t^{-p} \) is strictly increasing there (Chapter 17 §11). An operator monotone function is increasing (@prp-operator-monotone-increasing), so it is not operator monotone.
...
This pays Chapter 12 §05 and Chapter 16 §11 in full for the powers, and it is worth recording what it cost: @cor-shifted-inverse-operator-monotone from Chapter 16 §11, @lem-integral-preserves-loewner from the first part of this section, and facts (A1)–(A3). **Neither the theorem of Pick and Nevanlinna nor @thm-loewner itself was used** — Chapter 16 §11 expected the powers to arrive with Loewner's theorem, and they arrived without it. What (A7) adds is the knowledge that the representation exists for *every* operator monotone function; for the particular function \( t^{p} \) the representation was written down and verified by hand.
...
Fix \( t > 0 \). If \( t = 1 \) then \( f_p(1) = 0 = \log 1 \) for every \( p \). If \( t \ne 1 \), write \( c = \log t \ne 0 \) and \( x = pc \), so that \( t^{p} = e^{pc} = e^{x} \) by the definition of real powers (Chapter 17 §11) and
\[
...
Let \( \A, \B \in M_n(F) \) be Hermitian with spectra in \( (0, \infty) \), let \( \theta \in [0,1] \) and put \( \C = \theta\A + (1-\theta)\B \), whose spectrum again lies in \( (0, \infty) \), as Chapter 16 §11 checked.
...
Both proofs quote the fundamental theorem of calculus once, through the representation of \( \log t \) that the remark verified with it; nothing else outside Chapters 1–19 enters, and in particular @thm-pick-nevanlinna does not. With this, the two functions §08 left over are on its list.

The companion negative fact is the one Chapter 12 §05 named beside it.
...
::: {.remark}
The two halves of Chapter 12 §05's "companion fact" are now both in hand: \( \log \) is operator monotone on \( (0, \infty) \) (@cor-log-operator-monotone) and \( e^{t} \) is not operator monotone (@exm-exp-not-monotone). Neither proof used @thm-pick-nevanlinna. The logarithm and the exponential are inverse to each other and increasing, which shows that operator monotonicity is not inherited by inverse functions.
:::
