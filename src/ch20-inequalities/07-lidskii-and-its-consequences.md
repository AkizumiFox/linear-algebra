...
Specializing the gauge recovers both statements that Chapter 19 §10 wanted inside one: \( \Phi = \norm{\cdot}_\infty \) gives \( \max_i\lvert\sigma_i(\A)-\sigma_i(\B)\rvert \le \norm{\A-\B}_2 \), which is @cor-singular-value-perturbation, and \( \Phi = \norm{\cdot}_2 \) gives @thm-mirsky-frobenius. Chapter 19 §11's ledger entry — "the bounds of §05, and Mirsky's theorem in §10, hold in the Frobenius norm … their extension to every unitarily invariant norm belongs to Chapter 20" — is now paid for the eigenvalue bounds by @thm-lidskii-ui and for the singular value bounds by @thm-mirsky-ui. The clause elided above names §10's square-root and polar bounds (@thm-sqrt-perturbation, @thm-polar-positive-perturbation, @thm-polar-unitary-perturbation), and the promise reaches them as well. Those are **not** extended here: they still hold only in the two norms Chapter 19 proved them in, the spectral and the Frobenius.
...
Let \( \A, \B \in M_{m\times n}(\nC) \) and let \( \cH(\cdot) \) be the Hermitian dilation.
