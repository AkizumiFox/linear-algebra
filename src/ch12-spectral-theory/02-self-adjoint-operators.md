...
Chapter 11 already made the definition, so we only recall it: \( T \in \cL(V) \) is **self-adjoint** when \( T^{*} = T \), equivalently when
\[
...
The second theorem is the same move with two vectors instead of one, and it is the geometric half of the spectral theorem: the eigenspaces are not merely independent, as in Chapter 9, but perpendicular.
...
\]
so \( T^{*} = T \). In the standard basis, which *is* orthonormal, the matrix is \( \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix} \), symmetric as it must be. The moral is the one Chapter 11, Section 6 closes with: self-adjointness is a property of the operator and the inner product, never of a matrix taken on its own.
:::
...
Operators with the property in (a) are called **positive**, and they are the subject of Chapter 13; part (b) is the first half of the characterization proved there.
:::
