...
A triangular form with an explicit factorization makes two Chapter 9 facts fall out in a line each.
...
This is the tool behind a standard strategy: *prove it for diagonalizable matrices, then take a limit*. Chapter 16, where matrices acquire norms and limits can be spoken of properly, runs arguments of exactly this shape; and @exr-schur-triangularization-c1 uses the triangular form directly, with no limits at all, to reprove the Cayley–Hamilton theorem over \( \nC \).
