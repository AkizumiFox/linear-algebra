...
Sizes here are measured in the **Frobenius norm**, which Chapter 11 §07 defined as the norm induced by the Frobenius inner product:
\[
...
::: {.solution}
(a) \( \norm{\A}_F = \bigl(\sum_{i,j}\lvert a_{ij}\rvert^2\bigr)^{1/2} \), the norm induced by the Frobenius inner product \( \inner{\A}{\B} = \tr(\B^{*}\A) \) of Chapter 11. As a trace, \( \norm{\A}_F^2 = \tr(\A^{*}\A) \).
