...
Chapter 1 studied vector spaces one at a time. This chapter studies the functions **between** vector spaces that respect addition and scaling. This section defines them, collects examples from geometry, matrices, calculus and algebra, and proves the fact that makes them manageable: a linear map is completely determined by what it does to a basis.
...
Let \( X \) and \( Y \) be sets and \( g \colon X \to Y \) a function. Show that \( C_g \colon F^Y \to F^X \), \( C_g(h) = h \circ g \), is linear. Show also that \( \nR[x] \to \nR[x] \), \( p(x) \mapsto p(x + 1) \), is linear.
:::
...
For the second claim, write \( \sigma \colon \nR[x] \to \nR[x] \) for \( p(x) \mapsto p(x + 1) \); in full, if \( p = \sum_k a_kx^k \), then \( \sigma(p) = \sum_k a_k(x + 1)^k \), a finite sum in \( \nR[x] \). Linearity is now read off the coefficients. The coefficients of \( p + q \) are the \( a_k + b_k \), so \( \sigma(p + q) = \sum_k (a_k + b_k)(x + 1)^k = \sigma(p) + \sigma(q) \) by @thm-polynomial-ring-laws, and \( \sigma(cp) = \sum_k ca_k(x + 1)^k = c\,\sigma(p) \) in the same way. So \( \sigma \) is linear. (It translates the graph of \( p \) one unit to the left.) One can also read \( \sigma \) as an instance of \( C_g \) with \( g(t) = t + 1 \), once a polynomial and the function it defines are identified; that identification needs an argument, which Chapter 3 supplies.
:::
...
::: {.solution}
First, \( ((1, 1), (1, -1)) \) is a basis of \( \nR^2 \). It is independent: if \( a(1, 1) + b(1, -1) = \0 \), then \( a + b = 0 \) and \( a - b = 0 \), so \( a = b = 0 \). It has \( 2 = \dim \nR^2 \) entries, so it is a basis (@thm-right-size-basis). By @thm-linear-transform-basis, exactly one such \( T \) exists.
...
::: {.solution}
No. As in @exm-map-from-basis-values, \( ((1, 0), (1, 1)) \) is a basis of \( \nR^2 \): if \( a(1, 0) + b(1, 1) = \0 \) then \( b = 0 \) and \( a + b = 0 \), so \( a = b = 0 \), and two independent vectors in a \( 2 \)-dimensional space form a basis (@thm-right-size-basis). By @thm-linear-transform-basis, the first two conditions already determine \( T \), and the third condition must be checked against them, not imposed. Since \( (2, 3) = -1 \cdot (1, 0) + 3 \cdot (1, 1) \), any linear \( T \) with the first two values satisfies
\[
...
::: {.solution}
Solving \( a(1, 2) + b(2, 5) = (x, y) \), that is \( a + 2b = x \) and \( 2a + 5b = y \), gives \( b = y - 2x \) and \( a = 5x - 2y \). Every \( (x, y) \) has exactly one such expression, so \( ((1, 2), (2, 5)) \) is a basis of \( \nR^2 \). Hence \( T(x, y) = 3(5x - 2y) + 1 \cdot (y - 2x) = 13x - 5y \). Check: \( 13 - 10 = 3 \) and \( 26 - 25 = 1 \).
:::
...
::: {.solution}
(a) The list \( ((2, 1), (1, 1)) \) is independent: if \( a(2, 1) + b(1, 1) = \0 \), then \( 2a + b = 0 \) and \( a + b = 0 \), so \( a = 0 \) and then \( b = 0 \). It has \( 2 = \dim \nR^2 \) entries, so it is a basis (@thm-right-size-basis). By @thm-linear-transform-basis, exactly one such \( T \) exists. Solving \( a(2, 1) + b(1, 1) = (x, y) \), that is \( 2a + b = x \) and \( a + b = y \), gives \( a = x - y \) and \( b = 2y - x \). Hence
\[
