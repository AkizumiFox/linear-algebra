...
The examples of the last section kept producing the same coincidence. Differentiation \( \nR[x]_{\le 3} \to \nR[x]_{\le 2} \) has a kernel of dimension \( 1 \) and an image of dimension \( 3 \), and \( 1 + 3 = 4 = \dim \nR[x]_{\le 3} \). The trace on \( M_2(\nR) \) gives \( 3 + 1 = 4 \) again. This section proves that the same equation holds for every linear map on a finite-dimensional space, with no matrices at all, and then turns it into the most useful counting tool of the chapter. Chapter 3 meets the equation again for matrices, counted a different way.
...
Find the nullity and rank of (a) differentiation \( D \colon \nR[x]_{\le 3} \to \nR[x]_{\le 2} \); (b) the zero map and the identity on a space of dimension \( n \).
:::

::: {.solution}
(a) By @exm-differentiation-kernel-image, \( \ker D = \Span(1) \) and \( \im D = \nR[x]_{\le 2} \). So \( \nullity D = 1 \) and \( \rank D = 3 \).

(b) By @exm-kernel-image-degenerate, the zero map \( V \to V \) has nullity \( n \) and rank \( 0 \), and the identity has nullity \( 0 \) and rank \( n \).
:::
...
Applied to the matrix map \( T_\A \colon F^n \to F^m \), the theorem says \( \nullity T_\A + \rank T_\A = n \). In particular, **the solutions of a homogeneous system \( \A\x = \0 \) in \( n \) unknowns form a subspace of dimension \( n - \rank T_\A \)**, whatever method is used to find the rank. Chapter 3 proves this equation a second way, by counting the free and pivot columns of a reduced matrix; here we counted basis vectors, and no row operation appeared.
...
::: {.solution}
(a) \( T \) is linear by @thm-evaluation-respects-operations. *Kernel:* for \( p = a_0 + a_1x + a_2x^2 + a_3x^3 \), \( T(p) = \0 \) means \( a_0 = 0 \) and \( a_1 + a_2 + a_3 = 0 \), that is, \( p = a_2(x^2 - x) + a_3(x^3 - x) \). The list \( (x^2 - x, x^3 - x) \) spans \( \ker T \), and it is independent by @thm-distinct-degrees-independent, so it is a basis. *Image:* \( T(1) = (1, 1) \) and \( T(x) = (0, 1) \) are independent, so they span a \( 2 \)-dimensional subspace of \( \nR^2 \), which is \( \nR^2 \) (@thm-dim-impl-eq). So \( \im T = \nR^2 \) with basis \( ((1, 1), (0, 1)) \). Hence \( 2 + 2 = 4 = \dim \nR[x]_{\le 3} \).

(b) \( T \) is linear because the transpose is (@exm-trace-matrices). *Kernel:* \( \A + \A\tp = 0 \) means \( a_{11} = a_{22} = 0 \) and \( a_{21} = -a_{12} \), so \( \ker T = \Span(\E_{12} - \E_{21}) \), with basis \( (\E_{12} - \E_{21}) \). *Image:* each \( \A + \A\tp \) is symmetric, since \( (\A + \A\tp)\tp = \A\tp + \A \), and each symmetric \( \B \) equals \( T(\tfrac12 \B) \). So \( \im T \) is the space of symmetric matrices, with basis \( (\E_{11}, \E_{22}, \E_{12} + \E_{21}) \) (as in @exr-kernel-image-b2). Hence \( 1 + 3 = 4 = \dim M_2(\nR) \).
:::
