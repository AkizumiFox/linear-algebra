...
To know that \( T \) is invertible with inverse \( S \), the definition asks for two equations, \( ST = \id \) and \( TS = \id \). Often only one is easy to check. When is one enough? Not always: @exm-differentiation-integration-one-sided shows that it is not enough for operators on infinite-dimensional spaces. And it fails between spaces of different dimension: for \( T \colon \nR^2 \to \nR^3 \), \( (x, y) \mapsto (x, y, 0) \), and \( S \colon \nR^3 \to \nR^2 \), \( (x, y, z) \mapsto (x, y) \), we have \( ST = \id_{\nR^2} \) but \( TS(0, 0, 1) = \0 \), so \( TS \neq \id_{\nR^3} \). What survives is exactly the case that looks like "square":
...
::: {.remark}
The whole weight of the theorem falls on one step, "injective, hence bijective", and that step is Rank–Nullity and nothing else. Chapter 3 proves the matrix counterpart — if \( \A\B = \I_n \) for square \( \A \) and \( \B \), then \( \B\A = \I_n \) — by row reduction instead, reaching the same place by a different road. Once maps are written as matrices in Chapter 4, the two become one theorem seen from two sides.
:::
...
For an operator on a finite-dimensional space, invertibility can now be detected in many different ways, and it is worth having them in one place. The theorem below is the first of several such lists in this book, and the others all grow from it: Chapter 3 gives the matrix version, and later chapters add tests by determinant, by eigenvalue and by singular value.
...
Isomorphisms let us move a whole space into \( F^n \). Chapter 4 moves **maps** as well: once bases are chosen, every linear map between finite-dimensional spaces becomes a matrix, and composition becomes matrix multiplication.
