...
*Step 1: the weights.* \( \Delta_{n+1} \) is closed in \( \nR^{n+1} \) by @prp-closed-open-basics (b), (c), and bounded, since each coordinate of a point of it lies in \( [0, 1] \). By **the compactness of closed bounded sets in \( \nR^{n+1} \), fact (A3) of Chapter 16's introduction**, some subsequence of \( (\t_k) \) converges to a point \( \t = (t_0, \dots, t_n) \in \Delta_{n+1} \). In particular \( t_{k,i} \to t_i \) along it for each \( i \), since \( \lvert t_{k,i} - t_i\rvert \le \norm{\t_k - \t}_2 \).
...
(b) By @lem-linear-map-lipschitz, \( \varphi \) is continuous. By **the extreme value theorem, fact (A4) of Chapter 16's introduction**, applied to \( \varphi \) on the non-empty compact set \( K \), there is \( \x^{\ast} \in K \) with \( \varphi(\x) \le M = \varphi(\x^{\ast}) \) for every \( \x \in K \). Now let \( \y = \sum_i t_i\x_i \) be any point of \( \conv K \), with \( \x_i \in K \) and \( \t \in \Delta_m \) (@thm-convex-hull-combinations). By linearity, and since every \( t_i \ge 0 \),
\[
...
6. Which facts from Chapter 16's list of imported analysis does @cor-hull-of-compact-is-compact use, and where?
:::
::::
