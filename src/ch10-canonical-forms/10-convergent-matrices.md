...
::: {.remark}
**What we assume from analysis.** Beyond the four facts quoted in Chapter 9 §11, this section uses exactly the following.
...
So the only obstruction to a Markov chain settling down is an eigenvalue of modulus \( 1 \) other than \( 1 \) itself — periodicity, in the language of probability. Chapter 9's warning example, the swap \( \begin{pmatrix} 0&1\\1&0\end{pmatrix} \), has the eigenvalue \( -1 \) and is exactly this obstruction.
...
[C2: Chapter 9's Markov theorem without diagonalizability]
