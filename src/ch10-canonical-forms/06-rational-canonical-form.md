...
- **Step 1 (one prime at a time).** Chapter 9's primary decomposition splits \( V = V_1 \oplus \dots \oplus V_k \), where \( m_{T|_{V_i}} = p_i^{e_i} \) is a power of a single monic irreducible (@thm-primary-decomposition). Nothing more is needed from the general theory.
...
*The case \( e = 1 \).* Then \( m_T = p \) is irreducible. For \( \u \ne \0 \) we have \( m_{T,\u} \mid m_T = p \) (@thm-cyclic-subspace-basis (d)) and \( m_{T,\u} \ne 1 \), so \( m_{T,\u} = p \) and \( \dim Z(\u; T) = d \) by @thm-cyclic-subspace-basis (a). Two consequences.

*Every cyclic piece is minimal.* If \( W \subseteq Z(\u; T) \) is \( T \)-invariant and \( W \ne \{\0\} \), pick \( \w \in W \) with \( \w \ne \0 \). Then \( Z(\w; T) \subseteq W \) (@prp-cyclic-subspace-smallest (c)) and \( \dim Z(\w; T) = d = \dim Z(\u; T) \), so \( Z(\w; T) = Z(\u; T) \) by @thm-dim-impl-eq, and \( W = Z(\u; T) \). So the only \( T \)-invariant subspaces of \( Z(\u; T) \) are \( \{\0\} \) and \( Z(\u; T) \).

*The pieces fill \( V \).* Among all lists \( \u_1, \dots, \u_m \) of non-zero vectors for which \( Z(\u_1; T) + \dots + Z(\u_m; T) \) is direct, choose one with \( m \) as large as possible; this is possible because such a sum has dimension \( md \le \dim V \) (@thm-direct-sum-k-criteria). Write \( S \) for that sum. If \( S \ne V \), pick \( \v \in V \setminus S \). The subspace \( Z(\v; T) \cap S \) is \( T \)-invariant (@prp-invariant-sum-intersection), lies inside \( Z(\v; T) \) and does not contain \( \v \), so it is \( \{\0\} \) by the previous paragraph. Then \( S + Z(\v; T) \) is direct — a relation \( \c_1 + \dots + \c_m + \c = \0 \) with \( \c_j \in Z(\u_j; T) \) and \( \c \in Z(\v; T) \) puts \( \c \in Z(\v; T) \cap S = \{\0\} \), and directness of \( S \) kills the rest (@thm-direct-sum-k-criteria) — contradicting the maximality of \( m \). Hence \( V = Z(\u_1; T) \oplus \dots \oplus Z(\u_m; T) \) with every \( m_{T,\u_i} = p \), and every \( k_i = 1 = e \).

This settles \( e = 1 \), and in particular the base case \( n = 1 \), where \( m_T \) is linear.
