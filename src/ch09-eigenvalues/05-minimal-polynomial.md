...
Section 4 decided diagonalizability by counting eigenvectors, one eigenspace at a time. There is a second source of information about an operator, which Chapter 6 set up: the polynomials that kill it. On a finite-dimensional space they form an ideal of \( F[x] \), and that ideal has a single monic generator. This section gives the generator its name, the minimal polynomial, and shows that its roots are exactly the eigenvalues. We learn two ways to compute it and check that similar matrices share it. The next section compares it with the characteristic polynomial, and Section 8 reads diagonalizability off its factorization.

## The polynomial that divides all the others

Chapter 6 turned every polynomial identity satisfied by an operator into a statement about one ideal. For \( T \in \cL(V) \), the set
\[
I_T = \{ p \in F[x] : p(T) = 0 \}
\]
...
- **Projections.** Chapter 6 found, by this plan, that a projection \( P \) with \( P \ne 0 \) and \( P \ne \id_V \) has \( m_P = x^2 - x \) (the examples after @thm-annihilator-ideal).
...
- **Invertibility.** Taking \( \lambda = 0 \): for \( V \ne \{\0\} \), \( T \) is invertible if and only if \( m_T(0) \ne 0 \). Indeed \( m_T(0) = 0 \) if and only if \( 0 \in \spec(T) \), if and only if \( T = T - 0\,\id_V \) is not invertible (@thm-eigenvalue-characterizations, (a) \( \Leftrightarrow \) (c) with \( \lambda = 0 \); for matrices this is @thm-invertible-tfae-eigen). Chapter 6 proved the same statement without eigenvalues (@exr-polynomials-of-operators-c1 (c)).
...
Chapter 8 showed that a block diagonal matrix computes polynomials blockwise: \( p(\A \oplus \B) = p(\A) \oplus p(\B) \) (@thm-block-diagonal-arithmetic (b)). So a polynomial kills \( \A \oplus \B \) exactly when it kills both blocks. In the language of ideals, the answer is the least common multiple.
...
(The converse of (a), that \( \deg m_T = n \) forces a cyclic vector, is proved in Chapter 10.)
::::
