...
The previous section ended with an observation: an operator has a diagonal matrix exactly when the space splits into invariant lines, and a line \( \Span(\v) \) is invariant exactly when \( T\v \) is a multiple of \( \v \). This section turns that observation into the central notion of the chapter. We name the vectors that an operator only stretches, and the stretching factors. We find the factors as roots of the characteristic polynomial of Chapter 7, and we prove the two facts that everything later rests on: eigenvectors for different factors are independent, and over \( \nC \) every operator has at least one eigenvector.

## When is a matrix diagonal?

Chapter 4 asked how simple \( \mtx{T}{\sB}{\sB} \) can be made by choosing \( \sB \). The simplest square matrices are the diagonal ones, so start with the wish and unwind it.
...
\]
So the wish forces an equation: every basis vector must be sent to a multiple of itself. We met this in Chapter 2, where the reflection \( R(x, y) = (y, x) \) became \( \diag(1, -1) \) in the basis \( ((1, 1), (1, -1)) \) (@exm-reflection-diagonal-basis), precisely because \( R(1, 1) = (1, 1) \) and \( R(1, -1) = -(1, -1) \).
...
The definition asks for a non-zero vector in \( \ker(T - \lambda\,\id_V) \), a question about one \( \lambda \) at a time. In finite dimension, Chapters 2 and 7 turn it into a test that handles all \( \lambda \) at once. The last item is the one we compute with.
...
For matrices, this is @thm-charpoly-root-iff-singular read in the new language: Chapter 7 found the scalars \( c \) with \( c\I - \A \) singular, and those are the eigenvalues. It gives a procedure with two steps.
...
Chapter 2 collected the conditions equivalent to invertibility for an operator (@thm-invertible-operator-tfae), Chapter 3 gave the matrix form, the eight conditions of the Invertible Matrix Theorem (@thm-invertible-tfae), and Chapter 7 added the determinant (@thm-invertible-tfae-det). Eigenvalues give one more: a matrix is singular exactly when it kills a non-zero vector, which is to say, when it stretches some non-zero vector by the factor \( 0 \).
