...
This is the theorem several earlier chapters pointed to. Chapter 12 §11 described it as the statement "that for a diagonalizable \( \A \) every eigenvalue of a perturbed \( \A + \E \) lies within \( \kappa\norm{\E} \) of some eigenvalue of \( \A \), where \( \kappa \) measures how far from unitary the diagonalizing matrix is, and that \( \kappa = 1 \) exactly in the normal case", and said that it "needs matrix norms and a little analysis, and Chapter 20 develops both and proves it". Chapter 16 §07 and Chapter 17 §03 promised a bound "for a diagonalizable matrix" carrying "an extra factor that measures how far the eigenvector basis of \( \A \) is from orthonormal". Chapter 17 §11 left the bounds for non-Hermitian matrices, "beginning with Bauer–Fike", to this chapter, and Chapter 16 §08 recorded that "the bounds that do exist, Weyl's and Bauer–Fike's, are Chapters 17 and 20". We prove the theorem, identify the factor, prove the "exactly", and then return to the defective case. There the \( m \)-th root comes back, where \( m \) is the size of the largest Jordan block.
...
For a normal matrix the eigenvectors can be chosen orthonormal, and then the factor is \( 1 \). The second half of the next result proves the converse, in the form Chapter 12 §11 stated it: \( \kappa = 1 \) **exactly** in the normal case.
