...
The span theorem gives a clean proof of a fact from Chapter 3: a linear functional \( \varphi \) on \( M_n(F) \) with \( \varphi(\A \B) = \varphi(\B \A) \) is a multiple of the trace. Such a \( \varphi \) kills every commutator, hence their span, which is the kernel of the trace; @exr-commutators-and-shoda-c1 below completes the argument.
