...
::: {.idea}
For (a), rescale the coordinates by the entries of \( \x \). In the new coordinates, \( \x \) becomes \( \1 \), the hypothesis says that every row sum is at most \( s \), and Chapter 16's row-sum bound finishes the job. For (b), \( \x \) may have zero entries, so no rescaling is possible. Instead, pair the inequality with the left Perron vector \( \w > \0 \), which turns \( \w\tp\A\x \) into \( \rho\,\w\tp\x \).
:::
...
::: {.solution}
The row sums are \( 2, 3, 4 \) and the column sums are \( 1, 2, 6 \). By @cor-row-sum-bounds-rho (a), \( 2 \le \rho(\A) \le 4 \) from the rows and \( 1 \le \rho(\A) \le 6 \) from the columns. Keep the larger lower bound and the smaller upper bound: \( 2 \le \rho(\A) \le 4 \). Chapter 16's @cor-spectral-radius-row-column-bound gives only the upper bound \( 4 \).
...
\end{aligned}
\]
Exactly, \( p_{\A}(3.51) = -0.016849 < 0 \) and \( p_{\A}(3.512) = 0.004937728 > 0 \). A real polynomial that changes sign on an interval has a root there, by the intermediate value theorem, fact (A5) of Chapter 16's introduction. So \( p_{\A} \) has a real root between \( 3.51 \) and \( 3.512 \), consistent with the bracket.
:::

Two things about this computation deserve a comment. First, every number in it is a certified bound, not an approximation whose error is unknown: \( \rho(\A) \ge \tfrac{172}{49} \) is a theorem about this matrix, proved by one multiplication. Second, the iteration \( \x \mapsto \A\x \) is the power method, and Chapter 24 studies how fast it converges. Nesting alone does not make the brackets shrink to a point, as the next warning shows.
...
5. Name the result from Chapter 16 that gives the upper bound in @cor-row-sum-bounds-rho (a), and say what that result does not give.
:::
::::
...
\end{aligned}
\]
Then \( p_{\A}(\tfrac{35}{11}) = -\tfrac{498}{1331} < 0 \) and \( p_{\A}(\tfrac{13}{4}) = \tfrac{21}{64} > 0 \), so by the intermediate value theorem (fact (A5) of Chapter 16's introduction) \( p_{\A} \) has a root in the bracket, consistent with the estimate. (In fact \( \rho(\A) \approx 3.219 \).)
:::
