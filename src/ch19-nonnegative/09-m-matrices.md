...
Now the goal. For a Z-matrix \( \A = s\I - \B \) with \( s > 0 \), write \( \A = s(\I - s^{-1}\B) \). If \( \rho(\B) < s \), the Neumann series of Chapter 10 (@thm-neumann-series-spectral) inverts the second factor as \( \sum_k (s^{-1}\B)^k \), a sum of non-negative matrices. So a comparison between \( s \) and \( \rho(\B) \) is exactly what decides whether the inverse is non-negative, and it deserves a name.
...
The matrix \( \K_n^{-1} \) is a **discrete Green's function**: column \( j \) is the sag produced by a unit load at the single point \( t_j \), and positivity of every entry says that a load at one point moves every point of the string, in the same direction. The non-negativity of \( \K_n^{-1} \) is the discrete **maximum principle**: if \( \K_n\u \ge \0 \) then \( \u \ge \0 \). Since \( \K_n \) is symmetric, @cor-symmetric-m-matrix-positive-definite also shows \( \K_n \succ 0 \), which Chapter 7 foreshadowed by computing \( \det\K_n = n + 1 \) in @exr-special-determinants-b2.
...
- **Divide by the Perron root, then use Chapter 10.** Once \( \rho(\A) \) is known to be a simple eigenvalue dominating the others, \( \A/\rho(\A) \) meets the hypotheses of the convergence theorems of Chapter 10 §10, and no diagonalizability is ever needed (§05, Primitive matrices; §06, Markov chains). *Transfer:* normalize the dominant eigenvalue to \( 1 \) before asking about limits.
