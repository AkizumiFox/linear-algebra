...
Chapter 13 studied positive definite matrices at length, and the previous section showed that the signature is what congruence remembers. Putting the two together costs almost nothing and buys two things: a vocabulary that covers the forms Chapter 13 had no name for, the indefinite and the semidefinite ones, and the classical application that made quadratic forms worth studying in the first place. Nothing from Chapter 13 is reproved here. The field is \( \nR \) throughout, since none of the five words below means anything without an order.
...
Chapter 13 named two of the possible behaviors of \( q \): never negative, and always positive away from \( \0 \). There are three more, and the missing ones are not exotic. The form \( q(x_1, x_2) = x_1^2 - x_2^2 \) is positive at \( \e_1 \), negative at \( \e_2 \), and nothing in Chapter 13's vocabulary describes it.
...
So the five words sort the \( \tfrac12(n+1)(n+2) \) congruence classes of @cor-real-symmetric-classification into named families, and every one of them is decided by counting signs. Two of these five verdicts were already settled in Chapter 13 by other means, and it is worth putting the tests side by side.

::: {#prp-definiteness-tests-for-forms}
[Chapter 13's Tests, in the Language of Forms]
...
These are two standard theorems of multivariable calculus. This book proves neither. They are quoted so that the theorem below has hypotheses to stand on, and Chapter 18 quotes them again for its second-derivative test of convexity, exactly as the theorem of Abel and Ruffini was quoted in Chapter 11. Everything after them is linear algebra.
