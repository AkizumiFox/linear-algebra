...
For \( k = 1 \) this is @lem-extreme-eigenvalues-quadratic-form of Chapter 12: \( \lambda_1 \) is the largest value of \( \inner{\A\x}{\x} \) on the unit sphere. For \( k = n \) it degenerates in an instructive way: every \( \Q \) is unitary, every \( \Q^{*}\A\Q \) is unitarily similar to \( \A \), and the "maximum" is attained everywhere, at the common value \( \tr\A \). The content is in between.
...
## The promise from Chapter 12

Chapter 12 §10 proved a bound on how much of a matrix an orthonormal list can capture (@lem-orthonormal-capture-bound), and said of it: it "is the extreme case of a family of statements about sums of the largest eigenvalues of a Hermitian matrix; Chapter 16 develops the family through the Courant–Fischer min–max theorem". @thm-ky-fan is that family. Here is the lemma, recovered from it in four lines.
...
\]
So the largest singular value is the largest stretching factor, which is @thm-operator-norm-formulas (c), \( \norm{\A}_2 = \sigma_1(\A) \). Chapter 15 proved that from the singular value decomposition; here it is one line of Ky Fan.
