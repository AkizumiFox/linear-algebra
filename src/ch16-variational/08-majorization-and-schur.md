...
So \( \prec \) is reflexive and transitive. A relation with these two properties is called a **preorder**. Part (c) is antisymmetry up to rearrangement, and no more: \( (1, 0) \prec (0, 1) \prec (1, 0) \), yet \( (1, 0) \ne (0, 1) \). So \( \prec \) is **not** a partial order on \( \nR^n \). Restricted to **decreasing** vectors, where a rearrangement is the vector itself, (c) is exact antisymmetry, and there \( \prec \) is a partial order. By @exm-majorization-incomparable it is **not total**, not even among non-negative decreasing vectors with the same total. Like the Loewner order of Chapter 12 (@prp-loewner-partial-order), it leaves many pairs uncompared.
