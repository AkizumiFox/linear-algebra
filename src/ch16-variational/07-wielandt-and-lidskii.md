...
Chapter 8 §07 called a chain \( U_1 \subset U_2 \subset \dots \subset U_n = V \) with \( \dim U_k = k \) a **flag**, and a basis whose first \( k \) members span \( U_k \) a basis adapted to it. We need chains that skip dimensions, and lists that meet each member of the chain once.
...
- **An initial segment.** For the index set \( (1, 2, \dots, k) \), \( W_j \) has dimension \( j \). An adapted list is then an orthonormal basis of \( W_k \) whose first \( j \) vectors span \( W_j \), since \( j \) orthonormal vectors in the \( j \)-dimensional \( W_j \) span it. With \( k = n \) this is Chapter 8's complete flag.
