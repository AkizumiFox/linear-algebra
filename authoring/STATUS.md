...
- [x] `tools/check_forward_deps.py`'s `from_file` regex once read `ch24a-notation/index` as chapter 24, section 0 — i.e. *before* Chapter 24's own sections. Fixed: the letter is the middle component of the key, so the page is `(24, 'a', 0)` and sorts after every `(24, '', n)`. A label added there is now placed correctly
