...
- `\norm{\A}_F` and its unitary invariance (`lem-frobenius-unitarily-invariant`) live in Ch 10 §07, next to the unitary matrices the invariance is about; §11 recalls both. They were in §11 until the reading-path measurement showed that §11 joined seven of the ten published paths on that lemma alone. Ch 11 §01's exercise C2 re-proves it by hand and says so.
...
- **Four weakest links were cut**, each a whole section dragged onto many paths by one small citation. Ch 10 §06's adjoint-versus-dual-map theorem became exercise C4 (Ch 4 §§03–04 and Ch 1 §08 leave the reading path; SVD 81 → 78, spectral 73 → 70). Ch 11 §03's proof recalled `def-nilpotent` in a step it did not use it in; the recall moved into the statement (spectral 70 → 64, characters 95 → 93). `lem-frobenius-unitarily-invariant` moved from Ch 11 §11 to Ch 10 §07 (Perron 96 → 95, seven profiles one section shorter). `def-permutation-matrix` and `lem-permutation-matrices` (a)-(c) moved from Ch 2 §06 to Ch 2 §04, part (d) staying behind as `lem-swap-past-gauss-transform` because it is about this section's Gauss transforms (characters 95 → 94)
- **Left alone as real.** `thm-adjoint-exists` needs `thm-riesz-representation`, and Ch 10 §05 is on nine of the ten paths for that one proof; Ch 4 §§01–02 stay on the SVD path because `thm-annihilator-vs-orthogonal-complement` needs the annihilator, which is what §05 is for. Ch 9 §02 is a real prerequisite of the Jordan form and stays on the Jordan path
